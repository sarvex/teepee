...
use graphviz::maybe_owned_vec::{MaybeOwnedVector, Growable, Borrowed};
//use std::io::IoResult;
use std::ptr::{mod, RawPtr};
use std::fmt;
use std::io;
...
pub enum RawRequestTarget<'a> {
    ...
    /// written in.
    OriginForm(MaybeOwnedVector<'a, u8>),
    /// A complete URL; most commonly used with proxies, but all servers MUST
    /// support it for future compatibility.
    AbsoluteForm(MaybeOwnedVector<'a, u8>),
    /// Only used for `CONNECT` requests through proxies.
    AuthorityForm(MaybeOwnedVector<'a, u8>),
    /// Only used for a server-wide `OPTIONS` request.
    AsteriskForm,
}

impl<'a> fmt::Show for RawRequestTarget<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            OriginForm(ref v) => f.write(v.as_slice()),
            AbsoluteForm(ref v) => f.write(v.as_slice()),
            AuthorityForm(ref v) => f.write(v.as_slice()),
            AsteriskForm => f.write(b"*"),
        }
    }
}
...
impl<'a> RawRequestTarget<'a> {
    fn into_owned(self) -> RawRequestTarget<'static> {
        match self {
            OriginForm(Borrowed(slice)) => OriginForm(Growable(slice.to_vec())),
            AbsoluteForm(Borrowed(slice)) => AbsoluteForm(Growable(slice.to_vec())),
            AuthorityForm(Borrowed(slice)) => AuthorityForm(Growable(slice.to_vec())),
            OriginForm(Growable(vec)) => OriginForm(Growable(vec)),
            AbsoluteForm(Growable(vec)) => AbsoluteForm(Growable(vec)),
            AuthorityForm(Growable(vec)) => AuthorityForm(Growable(vec)),
...
/// Parser!
pub struct Parser<R: Reader, H: Handler> {
    inner: InnerBuffer<R>,
    handler: H,
}

impl<R: Reader, H: Handler> Parser<R, H> {
    ...
    pub fn parse(&mut self) -> Result<(), Error> {
        ...
        macro_rules! parse_byte {
            ($expected1:pat $(| $expected2:pat)*, $error:expr) => {
                match self.inner.take_byte() {
                    Ok(ok @ $expected1) $(| Ok(ok @ $expected2))* => ok,
        ...
        if try!(self.inner.take_bytes_while(is_tchar)) == 0 {
            parse_error!(BadMethod);
        }

        self.inner.set_marker1_end();

        let _ = parse_byte!(SP, BadMethod);
        ...
        let form = match b!() {
            b'/' if notable_method == NotableMethod::Connect => parse_error!(BadRequestTarget),
            b'/' => Form::Origin,
            b'*' if notable_method == NotableMethod::Options => Form::Asterisk,
            b'*' => parse_error!(BadRequestTarget),
            SP | HTAB | CR | LF => parse_error!(BadRequestTarget),
        ...
        let version = match self.inner.take_byte() {
            Ok(SP) => {
                let _ = parse_byte!(b'H', BadHttpVersion);
                let _ = parse_byte!(b'T', BadHttpVersion);
                let _ = parse_byte!(b'T', BadHttpVersion);
                let _ = parse_byte!(b'P', BadHttpVersion);
                let _ = parse_byte!(b'/', BadHttpVersion);
                let major = parse_byte!(b'0'...b'9', BadHttpVersion) - b'0';
                let _ = parse_byte!(b'.', BadHttpVersion);
                let minor = parse_byte!(b'0'...b'9', BadHttpVersion) - b'0';
                try!(self.inner.take_crlf(Some(BadHttpVersion)));
                (major, minor)
            },
            Ok(CR) => {
            ...
            Ok(LF) => (0, 9),
            Ok(_) => parse_error!(BadHttpVersion),
            Err(e) => return Err(e),
        };

        {
            ...
            let request_target = match form {
                ...
                Form::Origin => {
                    OriginForm(Borrowed(self.inner.get_marker2()))
                },
                Form::Authority => {
                    AuthorityForm(Borrowed(self.inner.get_marker2()))
                },
                Form::Absolute => {
                    AbsoluteForm(Borrowed(self.inner.get_marker2()))
                },
            };
        ...
        loop {
            ...
            match try!(self.inner.take_byte()) {
                ...
                LF => break,
                b if is_tchar(b) => (),
                _ => parse_error!(BadHeaderField),
            }
            let _ = try!(self.inner.take_bytes_while(is_tchar));
            self.inner.set_marker1_end();

            // ":" OWS
            let _ = parse_byte!(b':', BadHeaderField);
            ...
            loop {
                ...
                match try!(self.inner.peek_byte()) {
                    SP | HTAB => {
                        ...
                        if cr && lf {
                            self.inner.buf.as_mut_slice()[self.inner.pos - 2] = SP;
                        }
                        self.inner.buf.as_mut_slice()[self.inner.pos - 1] = SP;
                        self.inner.buf.as_mut_slice()[self.inner.pos] = SP;
...
struct InnerBuffer<R: Reader> {
    reader: R,
    /// The buffer around the reader, storing prepared data.
    buf: Vec<u8>,
    marker1_start: Option<uint>,
    marker1_end: Option<uint>,
    marker2_start: Option<uint>,
    marker2_end: Option<uint>,
    pos: uint,
}

impl<R: Reader> InnerBuffer<R> {
    ...
    #[inline]
    pub fn optionally_take_byte(&mut self, pred: |u8| -> bool) -> Result<bool, Error> {
    ...
    /// Returns `Err` if there is an error reading.
    #[inline]
    pub fn take_bytes_while(&mut self, pred: |u8| -> bool) -> Result<uint, Error> {
        let mut n = 0;
    ...
    pub fn peek_byte(&mut self) -> Result<u8, Error> {
        let byte = match self.buf.as_slice().get(self.pos) {
            Some(&byte) => byte,
    ...
    fn read_more_please(&mut self) -> Result<u8, Error> {
        ...
        match self.marker1_start {
            ...
            Some(old_marker) => {
                ...
                unsafe {
                    ...
                    let src = self.buf.as_ptr().offset(old_marker as int);
                    let len = self.pos;
        ...
        let bytes_read = match self.reader.read(self.buf.slice_from_mut(self.pos)) {
            Ok(bytes) => bytes,
            Err(io_error) => {
                unsafe { self.buf.set_len(self.pos) }
                return Err(IoError(io_error))
            },
        };
        assert!(bytes_read > 0);
        ...
        Ok(*unsafe { self.buf.as_slice().unsafe_get(self.pos) })
    }
}
...
pub trait Handler {
    ...
    fn on_body<R: Reader>(&mut self, reader: BodyReader<R>) -> ParserInstruction;
...
macro_rules! headers {
    ($($name:expr: $value:expr),*) => {{
        ...
        $(
            headers.insert_raw_line($name, $value);
        )*
        headers
    }};
    ($($name:expr: $value:expr),*,) => (headers!($($name:expr: $value:expr),*));
}
...
fn test_eager_request_parsing() {
    ...
    match parser.parse() {
        ...
        Err(e) => {
            panic!("huh!? {}", e)
            //println!("«{:?}»",
            //         ::std::str::from_utf8(parser.inner.buf.slice_from(parser.inner.pos))),
        }
    }
    //parser.parse().unwrap();
    assert_eq!(parser.handler, EagerRequest {
        method: ::method::Get,
        request_target: OriginForm(Borrowed(b"/")),
        http_version: (1, 1),
        headers: headers![
            "Header": b"value",
...
impl fmt::Show for EagerRequest {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        /*for &(ref name, ref value) in self.header_fields.iter() {
            try!(write!(f, "{}: ", *name));
            try!(f.write(value.as_slice()));
            try!(f.write(b"\n"));
        }*/
        try!(f.write(b"\n"));
        match self.body {
            None => try!(f.write(b"<no body>")),
            Some(ref body) => try!(f.write(body.as_slice())),
        }
        Ok(())
    }
}
...
impl Handler for EagerRequest {
    ...
    fn on_header_field(&mut self, field_name: Token, field_value: &[u8]) -> ParserInstruction {
        self.headers.insert_raw_line(field_name.to_string(), field_value.to_vec());
        ParserInstruction::Continue
    }

    fn on_body<R: Reader>(&mut self, _reader: BodyReader<R>) -> ParserInstruction {
        unimplemented!();
        //ParserInstruction::Continue
    }
