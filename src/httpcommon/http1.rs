...
const MAX_BUFFER_SIZE: uint = 0xffff;
...
impl<R: Reader, MT: MessageType::Impl, S: Step::Impl> Parser<R, MT, S> {
...
impl<R: Reader, MT: MessageType::Impl> Parser<R, MT, PreStartLine> {
...
impl<R: Reader> Parser<R, Request, PreStartLine> {
...
impl<R: Reader, MT: MessageType::Impl> Parser<R, MT, PostStartLine> {
...
impl<R: Reader, MT: MessageType::Impl, S: Step::Impl> Parser<R, MT, S> {
    ...
    fn get_working_space_one<'a>(&'a self) -> &'a [u8] {
        self.working_space.slice(0, self.ws_one_len as uint)
    }

    #[inline]
    /// Get the contents of the second item in the working space.
    fn get_working_space_two<'a>(&'a self) -> &'a [u8] {
        self.working_space.slice(self.ws_one_len as uint,
                                 self.ws_one_len as uint +
                                 self.ws_two_len as uint)
    }
    ...
    fn read_into_working_space(&mut self, start_point: uint,
                               rule: |u8| -> bool)
                              -> ParseResult<()> {
        for i in range(start_point, MAX_BUFFER_SIZE) {
            match self.reader.read_byte() {
                Ok(b) if rule(b) => {
                    // TODO: manipulate memory directly for perf’s sake.
                    // (i.e. drop bounds checking)
                    self.working_space[i] = b;
                },
                Ok(_) => return Ok(()),
                Err(_) => return Err(IoError),
            }
...
impl<R: Reader> Parser<R, Request, PostStartLine> {
...
impl<R: Reader> Parser<R, Response, PreStartLine> {
    ...
    pub fn read_status_line(mut self) -> ParseResult<(Parser<R, Response,
        ...
        let status = FromPrimitive::from_u16(
...
impl<R: Reader, MT: MessageType::Impl> Parser<R, MT, PreHeaderField> {
...
impl<R: Reader, MT: MessageType::Impl> Parser<R, MT, PostHeaderField> {
